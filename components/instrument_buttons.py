...
class InstrumentButtons(customtkinter.CTkFrame):
    def __init__(self, master, create_custom_instrument_callback, *args, **kwargs):
        ...
        self.piano_button = customtkinter.CTkButton(self, text="Piano", command=self.on_instrument_click, **button_config)
        self.piano_button.pack(side="left", padx=5)

        self.guitar_button = customtkinter.CTkButton(self, text="Guitar", command=self.on_instrument_click, **button_config)
